...
import './respone-email.css';

interface ResponeEmailProps {
  senderUsername : string,
  senderEmail : string,
  emailCreatedAtDate : string,
  selectedContent : string,
  emailFiles : {filename: string, filePath: string}[],
  closeResponeEmail: Void,
}

const ResponeEmail: React.FC<ResponeEmailProps> = ({ senderUsername, senderEmail, emailCreatedAtDate, selectedContent, emailFiles, closeResponeEmail }) => {
  
  const queryClient = useQueryClient();

  const [recipients] = useState<string[]>([senderEmail]);
  ...
  return (
    <form onSubmit={handleTransferEmail}>
      <div className='flex flex-col gap-3 p-3'>
        To: {recipients && recipients.length > 0 ? <div className='flex flex-row items-center gap-3 flex-wrap mt-3'>
        ...
        >
          ...
          <div>
            Date: {emailCreatedAtDate}, from: {senderUsername}({senderEmail}):
          </div>
