...
interface UpdateEmailProps {
  ...
  emailImgs : {url: string}[],
  closeUpdateEmail: Void,
}

const UpdateEmail: React.FC<UpdateEmailProps> = ({ emailID, emailRecipients, emailSubject, selectedContent, emailFiles, emailImgs, closeUpdateEmail }) => {
  ...
  const [recipients, setRecipients] = useState<string[]>([...emailRecipients]);
  ...
  const [selectedEmailFiles, setSelectedEmailFiles] = useState<{filename: string, filePath: string}[]>(emailFiles || []);
  ...
  const updateEmailMutation = useMutation(async (updateEmailFormData: {to: string[], emailContent: string | undefined, emailSubject: string, imgs: {url: string}[], files: {filename: string, filePath: string}[]}) => {
    const res = await axios.patch(`http://localhost:8800/api/v1/email/${emailID}`, updateEmailFormData, { headers: { Authorization: 'Bearer ' + Cookies.get('token') } });
    return res.data;
  }, {
    onSuccess: () => {
  ...
  function handleDeleteSelectedFile (index: number): void {
    let newSelectedFiles: {filename: string, filePath: string}[] = [];

    if (selectedEmailFiles) {
      for (let i = 0; i < selectedEmailFiles?.length; i++) {
        if (i !== index) {
          newSelectedFiles.push(selectedEmailFiles[i]);
        }
      }
    }

    setSelectedEmailFiles(newSelectedFiles);
  }
  ...
  async function handleUpdateEmail (event: FormEvent): Promise<void> {
    ...
    updateEmailMutation.mutate({
      ...
      emailSubject: '',
      files: [...selectedEmailFiles ,...uploadedFiles],
      imgs: [...emailImgs, ...imgs]
    });
  }
  ...
  return (
    <form onSubmit={handleUpdateEmail}>
      <div className='flex flex-col gap-3 p-3'>
        ...
        />
        <div 
          style={{ minHeight: '100px', maxHeight: '196px', resize: 'none' }} 
      ...
      {selectedEmailFiles.length > 0 ? (
        <div className='px-3 py-2 flex flex-row gap-3 overflow-x-auto'>
          {selectedEmailFiles.map((selectedEmailFile, index) => (
            <div key={index} className='flex items-center pr-2 rounded-full bg-blue-500'>
              <Icon
                ...
                customFunc={() => handleDeleteSelectedFile(index)} 
              />
              <a href={selectedEmailFile.filePath} className='text-gray-200'>
                {selectedEmailFile.filename}
              </a>
            </div>
          ))}
        </div>
